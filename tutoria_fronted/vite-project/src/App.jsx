      ...
      <div className="app">
        <Routes>
          ...
          <Route path="/admin/*" element={
            usuario?.rol === ROLES.ADMIN ? <AdminMenu usuario={usuario} onLogout={() => setUsuario(null)} /> :
            <Navigate to="/login" replace />
          } />
          <Route path="/profesor/*" element={
            usuario?.rol === ROLES.PROFESOR ? <ProfesorMenu usuario={usuario} onLogout={() => setUsuario(null)} /> :
            <Navigate to="/login" replace />
          } />
          <Route path="/tutor/*" element={
            usuario?.rol === ROLES.TUTOR ? <TutorMenu usuario={usuario} onLogout={() => setUsuario(null)} /> :
            <Navigate to="/login" replace />
          } />
          <Route path="/tutoriado/*" element={
            usuario?.rol === ROLES.TUTORIADO ? <TutoriadoMenu usuario={usuario} onLogout={() => setUsuario(null)} /> :
            <Navigate to="/login" replace />
          } />
        </Routes>
      </div>
